...
void antiaim::lagsync(CUserCmd* m_pcmd)
{
	...
	auto xDyaw = m_pcmd->m_viewangles.y;
...
float antiaim::get_pitch(CUserCmd* m_pcmd)
{
	...
	case 3:
		...
		break;
	case 4:
		pitch = math::random_float(-89.0f, 89.0f); //random
		break;
	case 5:
		pitch = invert_jitter ? 89.0f : -89.0f; //jitter
		break;
	/*case 6:
		pitch = g_cfg.misc.anti_untrusted ? 89.0f : 540.0f; //fake down
		break;
	case 7:
		pitch = g_cfg.misc.anti_untrusted ? -89.0f : -540.0f; //fake up
		break;*/
	case 6: //8
		pitch = invert_jitter ? (g_cfg.misc.anti_untrusted ? 89.0f : 540.0f) : (g_cfg.misc.anti_untrusted ? -89.0f : -540.0f); //fake jitter
		break;
	}
...
void antiaim::pitch_exploit(CUserCmd* m_pcmd)
{
	...
	if (&g_cfg.antiaim.pitch_exploit_key)
	{
		float pitch = 89.0f;

		if (m_pcmd->m_tickcount % 2 == 0)
			pitch = -89.0f;
