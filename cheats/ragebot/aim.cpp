...
//NEW 4 players, knife 100, scar 30
...
bool aim::automatic_stop(CUserCmd* cmd)
{
	...
	else
	{
		...
		auto speed = engineprediction::get().backup_data.velocity.Length2D();
		...
		Vector stop = forward * -speed;

		cmd->m_forwardmove = stop.x;
		cmd->m_sidemove = stop.y;
...
void aim::fire(CUserCmd* cmd)
{
	...
	if (net_channel_info)
	{
		...
		auto max_tickbase_shift = m_gamerules()->m_bIsValveDS() ? 2 : 16; // 1034 aim.cpp // 2:16 (6:16)
		//если чё убрать
	...
	log << crypt_str("Attemp Hit ") + (std::string)player_info.szName + crypt_str(": ");
