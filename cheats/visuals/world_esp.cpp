...
void worldesp::molotov_timer(entity_t* entity)
{
	...
	render::get().text(fonts[GRENADES], screen_origin.x + 1.0f, screen_origin.y - 20.5f, Color(220, 220, 220, 245), HFONT_CENTERED_X | HFONT_CENTERED_Y, "l");
...
void worldesp::smoke_timer(entity_t* entity)
{
	...
	render::get().text(fonts[GRENADES], screen_origin.x + 1.0f, screen_origin.y - 19.0f, g_cfg.esp.smoke_timer_color, HFONT_CENTERED_X | HFONT_CENTERED_Y, "k");
...
void worldesp::PlayerTrails()
{
	if (g_ctx.local()->is_alive() && m_engine()->IsInGame() && m_engine()->IsConnected())
	{
		...
		DrawBeam(local_pos, Vector(local_pos.x, local_pos.y + 10, local_pos.z), g_cfg.esp.trails_color);
	}
}
