...
void misc::watermark()
{
	...
	std::string name_cheat = crypt_str("artic");
#ifdef _DEBUG
	...
	if (m_engine()->IsInGame())
	{
		...
		if (nci)
		{
			...
			watermark = name_cheat + crypt_str("v1.2.2") + crypt_str(" | ") + server + crypt_str(" | ping: ") + std::to_string(g_ctx.globals.ping) + crypt_str(" ms | ") + tickrate + crypt_str(" tick | ") + g_ctx.globals.time;
		}
	}
...
void misc::ChatSpamer()
{
	...
	{
		crypt_str("artic - good cheat for the game."),
		crypt_str("Get good. Get artic."),
		crypt_str("artic - better than its competitors."),
		crypt_str("Go to another level with artic."),
		crypt_str("artic - free software."),
	};
	...
	if (GetTickCount() - lastspammed > 800)
	{
		...
		std::string msg = crypt_str("say ") + chatspam[rand() % 5];
...
void misc::TrashTalk() {
	...
	static std::string trashtalk_msg[] = {
		crypt_str(u8"я твою мамашу топором хуярил"),
		crypt_str(u8"Why so ez)"),
		crypt_str(u8"Delete rage, play legit"),
		crypt_str(u8"Delete cs,  play valorant"),
		crypt_str(u8"1 глупый дог"),
		crypt_str(u8"пока ты сидел на работе я строил пизанскую башнб в клиторе твоей троепроблядской мамаши 1"),
		crypt_str(u8"я твою овцу сьем прямо у тебя перед глазами"),
		crypt_str(u8"я сожру твою овцу у тебя на глазах"),
		crypt_str(u8"еееей биля как там мама ебаный в рот"),
		crypt_str(u8"арбуз ебаный"),
		crypt_str(u8"я тебе щас нос в голову вобью, хочешь?"),
		crypt_str(u8"давай сын шлюхи вытирай слезы и продолжай мне сосать"),
		crypt_str(u8"ебаный ты кукашонок втулись в мои яйца своей тугой еблиной"),
		crypt_str(u8"когда матери новые трусы купишь после того как мои братья протаранили ее анальное отверстие, порвав ее партки ебаные???"),
		crypt_str(u8"Что обычно подают к мексиканской текиле? Правильно, твою мамашу"),
		crypt_str(u8"Uhh, daddy, cum ples"),
		crypt_str(u8"че с ебалом нахуй нищинка ебаная?"),
		crypt_str(u8"ezz man del game or buy gamesense"),
		crypt_str(u8"нахуй ты лезешь иди убейся"),
		crypt_str(u8"бош крип уйди отсюда нахуй"),
		crypt_str(u8"ебанашка че ты без скита тут забыл?"),
		crypt_str(u8"скыт топ чыт"),
		crypt_str(u8"купи софтину или так и будешь падать"),
		crypt_str(u8"чел тебя фри хак ебет"),
		crypt_str(u8"жду оправдания в мой хуй"),
		crypt_str(u8"не хуево хснул тебя бича?"),
		crypt_str(u8"я на очке твоей матери форсаж 15 снимал"),
		crypt_str(u8"единичкой упал пидарас"),
		crypt_str(u8"мусор ливни с сервера"),
		crypt_str(u8"удали свою ру пасту"),
		crypt_str(u8"что не дает тебе не кого убить ? Artic"),
		crypt_str(u8"Извините за нецензурную брань. Но ты упал башкой об пол, 1, сын поноса!"),
		crypt_str(u8"жирный рейв трип юзер, я тебе в некст раунде нос сломаю,чтоб не визжал от радости, 1"),
	};

	srand(m_globals()->m_tickcount);
	std::string msg = crypt_str("say ") + trashtalk_msg[rand() % 33];
...
bool misc::double_tap(CUserCmd* m_pcmd)
{
	...
	auto recharge_time = g_ctx.globals.weapon->can_double_tap() ? TIME_TO_TICKS(0.01f) : TIME_TO_TICKS(0.01f);
...
void misc::fakeping()
{
	int value = g_cfg.misc.extended_backtracking_value / 2;
