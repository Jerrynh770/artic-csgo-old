...
//  This is the time to examine the usercmd for anything extra.  This call happens even if think does not.
	...
	void predict(CUserCmd* cmd, player_t* player)
	{
		...
		m_globals()->m_curtime = TICKS_TO_TIME(g_ctx.globals.fixed_tickbase); //TODO: need test //g_ctx.globals.fixed_tickbase = TICKS_TO_TIME(player->m_nTickBase());
		m_globals()->m_frametime = m_prediction()->EnginePaused ? 0.f : m_globals()->m_intervalpertick;
		m_globals()->m_tickcount = g_ctx.globals.fixed_tickbase;//player->m_nTickBase(); //TODO: need test
	...
	//{
	//	if (!m_engine()->IsInGame() || !g_ctx.local() || !g_ctx.local()->is_alive() || g_ctx.local()->m_hViewModel() == 0xFFFFFFFF)
	//		return;

	//	C_BaseViewModel* view_model = reinterpret_cast<C_BaseViewModel*>(i::entitylist->get_client_entity_from_handle((CBaseHandle)g_ctx.local()->m_hViewModel()));
	//	if (!view_model)
	//		return;
	...
	void ProcessInterpolation(bool bPostFrame)
	{
		...
		if (!bPostFrame)
		{
			...
			int m_iTickCount = g_ctx.globals.fakeducking ? (14 - m_clientstate()->iChokedCommands) : m_globals()->m_tickcount;
			g_ctx.local()->m_nFinalPredictedTick() = m_iTickCount + TIME_TO_TICKS(outgoing + incoming);
			return;
		}
