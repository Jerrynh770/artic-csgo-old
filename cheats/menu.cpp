...
std::string get_config_dir()
{
	...
	if (SUCCEEDED(SHGetFolderPath(NULL, 0x001a, NULL, NULL, path)))
		folder = std::string(path) + crypt_str("\\artic\\Configs\\");
...
void draw_multicombo(const char* name, std::vector<int>& variable, const char* labels[], int count, std::string& preview)
{
	...
	if (ImGui::BeginCombo(name, preview.c_str())) // draw start
	{
		ImGui::Spacing();
		ImGui::BeginGroup();
		{
...
void lua_edit(std::string window_name)
{
	...
	auto get_dir = [&]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
			file_path = std::string(path) + crypt_str("\\artic\\lua\\");
	...
	if (ImGui::CustomButton(crypt_str("Save"), (crypt_str("Save") + window_name).c_str(), ImVec2(125 * (1.f + ((c_menu::get().dpi_scale - 1.0) * 0.5f)), 0), true))
	{
		std::ofstream out;
...
void c_menu::draw(bool is_open)
{
	...
	if (g_cfg.menu.blur)
		PostProcessing::performFullscreenBlur(ImGui::GetBackgroundDrawList(), m_alpha);
	

	// start menu render
	ImGui::Begin(crypt_str("artic1"), nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoBackground);
	{
		auto p = ImGui::GetWindowPos();
		pos = ImGui::GetWindowPos();
		draw = ImGui::GetWindowDrawList();
		ImGuiColorEditFlags misc_flags = ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_NoOptions | ImGuiColorEditFlags_NoLabel;
		draw->AddRectFilled(ImVec2(pos.x, pos.y), ImVec2(pos.x + 900 * dpi_scale, pos.y + 620 * dpi_scale), ImGui::GetColorU32(ImGuiCol_WindowBg), style->WindowRounding);
		draw->AddRectFilled(ImVec2(pos.x + 185 * dpi_scale, pos.y), ImVec2(pos.x + 900 * dpi_scale, pos.y + 54 * dpi_scale), ImGui::GetColorU32(ImGuiCol_Additional), style->WindowRounding, ImDrawCornerFlags_TopRight);
		draw->AddRectFilled(pos, ImVec2(pos.x + 185 * dpi_scale, pos.y + 620 * dpi_scale), ImGui::GetColorU32(ImGuiCol_Additional), style->WindowRounding, ImDrawCornerFlags_Left);

		ImGui::PushFont(bold);
		//draw->AddImage(logo, { pos.x + 68 * dpi_scale, pos.y + 23 * dpi_scale }, { pos.x + 112 * dpi_scale, pos.y + 68 * dpi_scale });
		draw->AddText({ pos.x - 30 * dpi_scale + ImGui::CalcTextSize("ARTIC3").x, pos.y + 45 * dpi_scale }, ImGui::GetColorU32(ImGuiCol_Active), "  A R T I C ");
		draw->AddText({ pos.x + 44 * dpi_scale, pos.y + 45 * dpi_scale }, ImGui::GetColorU32(ImGuiCol_Text), "  A R T I C ");
		ImGui::PopFont();

		static int tabs = 0;
		static int subtab = 0;
		ImGui::PushFont(tab);
		draw->AddText({ pos.x + 21 * dpi_scale, pos.y + 120 * dpi_scale }, ImGui::GetColorU32(ImGuiCol_Text) - ImGui::GetColorU32(ImVec4(0.15f, 0.15f, 0.15f, 0.f)), "AIMBOT");
		draw->AddText({ pos.x + 21 * dpi_scale, pos.y + 287 * dpi_scale }, ImGui::GetColorU32(ImGuiCol_Text) - ImGui::GetColorU32(ImVec4(0.15f, 0.15f, 0.15f, 0.f)), "VISUALS");
		draw->AddText({ pos.x + 21 * dpi_scale, pos.y + 455 * dpi_scale }, ImGui::GetColorU32(ImGuiCol_Text) - ImGui::GetColorU32(ImVec4(0.15f, 0.15f, 0.15f, 0.f)), "MISC");
		ImGui::PopFont();
		...
		ImGui::SetCursorPos(ImVec2(13 * dpi_scale, 147 * dpi_scale)); ImGui::BeginGroup();
		{
			if (ImGui::Tab("Ragebot", 1, active_tab == 0, ImVec2(158 * dpi_scale, 40 * dpi_scale)))
				active_tab = 0;
			if (ImGui::Tab("Anti-Aim", 2, active_tab == 1, ImVec2(158 * dpi_scale, 40 * dpi_scale)))
				active_tab = 1;
			//if (ImGui::Tab("Legitbot", 3, active_tab == 2, ImVec2(158 * dpi_scale, 40 * dpi_scale)))
			//	active_tab = 2;
		} ImGui::EndGroup();

		ImGui::SetCursorPos(ImVec2(13 * dpi_scale, 315 * dpi_scale)); ImGui::BeginGroup();
		{
			if (ImGui::Tab("Main", 4, active_tab == 3, ImVec2(158 * dpi_scale, 40 * dpi_scale)))
				active_tab = 3;
			if (ImGui::Tab("Players", 5, active_tab == 4, ImVec2(158 * dpi_scale, 40 * dpi_scale)))
				active_tab = 4;
			if (ImGui::Tab("Skins", 6, active_tab == 5, ImVec2(158 * dpi_scale, 40 * dpi_scale)))
				active_tab = 5;
		} ImGui::EndGroup();

		ImGui::SetCursorPos(ImVec2(13 * dpi_scale, 487 * dpi_scale)); ImGui::BeginGroup();
		{
			if (ImGui::Tab("Main ", 7, active_tab == 6, ImVec2(158 * dpi_scale, 40 * dpi_scale)))
				active_tab = 6;
			if (ImGui::Tab("Config", 8, active_tab == 7, ImVec2(158 * dpi_scale, 40 * dpi_scale)))
				active_tab = 7;
		}
		...
		{
			static int subtab = 0;
			ImGui::SetCursorPos(ImVec2(186 * dpi_scale, 0)); ImGui::BeginGroup();
			{

				if (ImGui::SubTab("General", subtab == 0, ImVec2(94 * dpi_scale, 53* dpi_scale)))
					subtab = 0;
				ImGui::SameLine(100 * dpi_scale, 0);
				if (ImGui::SubTab("Weapon", subtab == 1, ImVec2(94 * dpi_scale, 53* dpi_scale)))
					subtab = 1;
			}
			ImGui::EndGroup();
			if (subtab == 0)
			{
				 ImGui::SetCursorPos(ImVec2(202* dpi_scale, 71* dpi_scale));
				ImGui::BeginChild("##RAGE1_FIRST", ImVec2(332 * dpi_scale, 534 * dpi_scale), false);
				{
					...
					ImGui::Checkbox(crypt_str("Autofire"), &g_cfg.ragebot.autoshoot);
					ImGui::Checkbox(crypt_str("Autoscope"), &g_cfg.ragebot.autoscope);
					ImGui::Spacing();
					...
					if (g_cfg.ragebot.double_tap)
					{
						...
						ImGui::Checkbox(crypt_str("Defensive"), &g_cfg.ragebot.peej);
					}
				...
				ImGui::EndChild();
				 ...
				 ImGui::SetCursorPos(ImVec2(550* dpi_scale, 71* dpi_scale));
				ImGui::BeginChild("##RAGE2_FIRST", ImVec2(332 * dpi_scale, 534 * dpi_scale), false);
				{
					ImGui::SetCursorPosY(5);
			...
			{
				 ImGui::SetCursorPos(ImVec2(202* dpi_scale, 71* dpi_scale));
				ImGui::BeginChild("##RAGE1_SECOND", ImVec2(332 * dpi_scale, 534 * dpi_scale), false);
				{
...
#if BETA
					ImGui::Checkbox(crypt_str("Point scale"), &g_cfg.ragebot.weapon[hooks::rage_weapon].static_point_scale);
				...
				ImGui::EndChild();
				 ...
				 ImGui::SetCursorPos(ImVec2(550* dpi_scale, 71* dpi_scale));
				ImGui::BeginChild("##RAGE2_SECOND", ImVec2(332 * dpi_scale, 534 * dpi_scale), false);
				{
					ImGui::SetCursorPosY(5);
					ImGui::Checkbox(crypt_str("Autostop"), &g_cfg.ragebot.weapon[hooks::rage_weapon].autostop);
					...
					if (g_cfg.ragebot.weapon[hooks::rage_weapon].hitchance)
						ImGui::SliderInt(crypt_str("Hitchance amount"), &g_cfg.ragebot.weapon[hooks::rage_weapon].hitchance_amount, 0, 101);
					...
					if (g_cfg.ragebot.autowall)
						ImGui::SliderInt(crypt_str("Autowall damage"), &g_cfg.ragebot.weapon[hooks::rage_weapon].minimum_damage, 1, 120, true);
		...
		if (active_tab == 1)
		{
			...
			ImGui::SetCursorPos(ImVec2(186 * dpi_scale, 0)); ImGui::BeginGroup();
			{
				...
				if (ImGui::SubTab("Anti-Aim", subtab == 2, ImVec2(94 * dpi_scale, 53* dpi_scale)))
					subtab = 2;
			}
			ImGui::EndGroup();
			if (subtab == 2)
			{
				 ImGui::SetCursorPos(ImVec2(202* dpi_scale, 71* dpi_scale));
				ImGui::BeginChild("##AA1_FIRST", ImVec2(332 * dpi_scale, 534 * dpi_scale), false);
				{
					ImGui::SetCursorPosY(5);
					static auto type = 0;
				...
				ImGui::EndChild();
				 ...
				 ImGui::SetCursorPos(ImVec2(550* dpi_scale, 71* dpi_scale));
				ImGui::BeginChild("##AA2_FIRST", ImVec2(332 * dpi_scale, 534 * dpi_scale), false);
				{
					ImGui::SetCursorPosY(5);
		...
		if (active_tab == 2)
		{
			...
			ImGui::SetCursorPos(ImVec2(186 * dpi_scale, 0)); ImGui::BeginGroup();
			{
				...
				if (ImGui::SubTab("LEGIT", subtab == 3, ImVec2(94 * dpi_scale, 53* dpi_scale)))
					subtab = 3;
			}
			ImGui::EndGroup();
			if (subtab == 3)
			{
				 ImGui::SetCursorPos(ImVec2(202* dpi_scale, 71* dpi_scale));
				ImGui::BeginChild("##LEGIT1_FIRST", ImVec2(332 * dpi_scale, 534 * dpi_scale), false);
				{
					ImGui::SetCursorPosY(5);
					...
					draw_keybind(crypt_str("Enable Legitbot"), &g_cfg.legitbot.key, crypt_str("##HOTKEY_LGBT_KEY"));
				...
				ImGui::EndChild();
				 ...
				 ImGui::SetCursorPos(ImVec2(550* dpi_scale, 71* dpi_scale));
				ImGui::BeginChild("##LEGIT2_FIRST", ImVec2(332 * dpi_scale, 534 * dpi_scale), false);
				{
					ImGui::SetCursorPosY(5);
		...
		if (active_tab == 3)
		{
			...
			ImGui::SetCursorPos(ImVec2(186 * dpi_scale, 0)); ImGui::BeginGroup();
			{
				...
				if (ImGui::SubTab("General", subtab == 4, ImVec2(94 * dpi_scale, 53* dpi_scale)))
					subtab = 4;
				ImGui::SameLine(100 * dpi_scale, 0);
				if (ImGui::SubTab("World", subtab == 5, ImVec2(94 * dpi_scale, 53* dpi_scale)))
					subtab = 5;
			}
			ImGui::EndGroup();
			if (subtab == 4)
			{
				 ImGui::SetCursorPos(ImVec2(202* dpi_scale, 71* dpi_scale));
				ImGui::BeginChild("##VISUALS1_FIRST", ImVec2(332 * dpi_scale, 534 * dpi_scale), false);
				{
					ImGui::SetCursorPosY(5);
					...
					if (g_cfg.esp.grenade_warning) {
						...
						ImGui::BeginGroup(); {//icon и другая хуйня
							...
							if (g_cfg.warning.main.enable) {
								...
								ImGui::Checkbox("Show background", &g_cfg.warning.main.show_bg);
				...
				ImGui::EndChild();
				 ...
				 ImGui::SetCursorPos(ImVec2(550* dpi_scale, 71* dpi_scale));
				ImGui::BeginChild("##VISUALS2_FIRST", ImVec2(332 * dpi_scale, 534 * dpi_scale), false);
				{
					ImGui::SetCursorPosY(5);
			...
			{
				 ImGui::SetCursorPos(ImVec2(202* dpi_scale, 71* dpi_scale));
				ImGui::BeginChild("##VISUALS1_SECOND", ImVec2(332 * dpi_scale, 534 * dpi_scale), false);
				{
					ImGui::SetCursorPosY(5);
				...
				ImGui::EndChild();
				 ...
				 ImGui::SetCursorPos(ImVec2(550* dpi_scale, 71* dpi_scale));
				ImGui::BeginChild("##VISUALS2_SECOND", ImVec2(332 * dpi_scale, 534 * dpi_scale), false);
				{
					ImGui::SetCursorPosY(5);
		...
		if (active_tab == 4)
		{
			...
			ImGui::SetCursorPos(ImVec2(186 * dpi_scale, 0)); ImGui::BeginGroup();
			{
				...
				if (ImGui::SubTab("Enemy", subtab == 6, ImVec2(94 * dpi_scale, 53* dpi_scale)))
					subtab = 6;
				ImGui::SameLine(100 * dpi_scale, 0);
				if (ImGui::SubTab("Team", subtab == 7, ImVec2(94 * dpi_scale, 53* dpi_scale)))
					subtab = 7;
				ImGui::SameLine(200 * dpi_scale, 0);
				if (ImGui::SubTab("Local", subtab == 8, ImVec2(94 * dpi_scale, 53* dpi_scale)))
					subtab = 8;
			...
			case 8:
			 ...
			 ImGui::SetCursorPos(ImVec2(202* dpi_scale, 71* dpi_scale));
			ImGui::BeginChild("##PLAYERS1_FIRST", ImVec2(332 * dpi_scale, 534 * dpi_scale), false);
			{
				ImGui::SetCursorPosY(5);
		...
		"Maximus" };
				...
				//draw_combo(crypt_str("Mask Changer"), g_cfg.player.maskchanger, maskchanger, ARRAYSIZE(maskchanger));
				}				
			...
			ImGui::EndChild();
			 ...
			 ImGui::SetCursorPos(ImVec2(550* dpi_scale, 71* dpi_scale));
			ImGui::BeginChild("##PLAYERS2_FIRST", ImVec2(332 * dpi_scale, 534 * dpi_scale), false);
			{
				ImGui::SetCursorPosY(5);
			...
			ImVec2 vecWindowPosition = ImGui::GetWindowPos() + ImVec2(92 * c_menu::get().dpi_scale, 74 * c_menu::get().dpi_scale);

			ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(vecWindowPosition.x + (654 + 150) * c_menu::get().dpi_scale, vecWindowPosition.y - 74 * c_menu::get().dpi_scale), ImVec2(vecWindowPosition.x + 1100 * c_menu::get().dpi_scale, vecWindowPosition.y + (620 - 74) * c_menu::get().dpi_scale), ImGui::GetColorU32(ImGuiCol_WindowBg));

			ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(vecWindowPosition.x + (654 + 150) * c_menu::get().dpi_scale, vecWindowPosition.y - 74 * c_menu::get().dpi_scale), ImVec2(vecWindowPosition.x + 1100 * c_menu::get().dpi_scale, vecWindowPosition.y + (54 - 74) * c_menu::get().dpi_scale), ImGui::GetColorU32(ImGuiCol_Additional), style->WindowRounding, ImDrawCornerFlags_TopRight);


			ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(vecWindowPosition.x + (654 + 165) * c_menu::get().dpi_scale, vecWindowPosition.y - 3 * c_menu::get().dpi_scale), ImVec2(vecWindowPosition.x + 1070 * c_menu::get().dpi_scale, vecWindowPosition.y + (603 - 74) * c_menu::get().dpi_scale), ImGui::GetColorU32(ImGuiCol_Additional), style->ChildRounding);
			ImGui::PushFont(tab);
			ImGui::GetForegroundDrawList()->AddText(ImVec2(vecWindowPosition.x + (654 + 247) * c_menu::get().dpi_scale, vecWindowPosition.y + 15 * c_menu::get().dpi_scale), ImColor(255, 255, 255, 255), "ESP PREVIEW");
			ImGui::PopFont();

			vecWindowPosition = ImGui::GetWindowPos() + ImVec2(92 * c_menu::get().dpi_scale, 114 * c_menu::get().dpi_scale);
			static int curnet_model = 0;
			if (g_DrawModel->GetTexture())
			{
				ImGui::GetForegroundDrawList()->AddImage(
					g_DrawModel->GetTexture()->pTextureHandles[0]->lpRawTexture,
					ImVec2(vecWindowPosition.x + 760 * c_menu::get().dpi_scale, vecWindowPosition.y - 130 * c_menu::get().dpi_scale),
					ImVec2(vecWindowPosition.x + (760 + g_DrawModel->GetTexture()->GetActualWidth()) * c_menu::get().dpi_scale, vecWindowPosition.y + (g_DrawModel->GetTexture()->GetActualHeight() - 130) * c_menu::get().dpi_scale),
			...
			vecWindowPosition = ImGui::GetWindowPos() + ImVec2(92 * c_menu::get().dpi_scale, 74 * c_menu::get().dpi_scale);

			Color aBox = Color(g_cfg.player.type[0].box_color);
			if (g_cfg.player.type[0].box)
			{
				ImGui::GetForegroundDrawList()->AddRect(ImVec2(vecWindowPosition.x + 844 * c_menu::get().dpi_scale, vecWindowPosition.y + 79 * c_menu::get().dpi_scale), ImVec2(vecWindowPosition.x + (886 + 170) * c_menu::get().dpi_scale, vecWindowPosition.y + (386 + 40) * c_menu::get().dpi_scale), ImColor(0, 0, 0, 255));
				ImGui::GetForegroundDrawList()->AddRect(ImVec2(vecWindowPosition.x + 846 * c_menu::get().dpi_scale, vecWindowPosition.y + 81 * c_menu::get().dpi_scale), ImVec2(vecWindowPosition.x + (884 + 170) * c_menu::get().dpi_scale, vecWindowPosition.y + (384 + 40) * c_menu::get().dpi_scale), ImColor(0, 0, 0, 255));
				ImGui::GetForegroundDrawList()->AddRect(ImVec2(vecWindowPosition.x + 845 * c_menu::get().dpi_scale, vecWindowPosition.y + 80 * c_menu::get().dpi_scale), ImVec2(vecWindowPosition.x + (885 + 170) * c_menu::get().dpi_scale, vecWindowPosition.y + (385 + 40) * c_menu::get().dpi_scale), ImColor(aBox.r(), aBox.g(), aBox.b(), aBox.a()));
			}
			...
			if (g_cfg.player.type[0].name)
			{
				...
				ImGui::GetForegroundDrawList()->AddText(ImVec2(vecWindowPosition.x + (715 + 150 + 82) * c_menu::get().dpi_scale - ImGui::CalcTextSize((wagasas[model_type])).x / 2, vecWindowPosition.y + 54), ImColor(aName.r(), aName.g(), aName.b(), aName.a()), (wagasas[model_type]));

			}
			Color aHealthBar = Color(g_cfg.player.type[0].health_color);
			if (g_cfg.player.type[0].health)
			{
				ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(vecWindowPosition.x + (689 + 150) * c_menu::get().dpi_scale, vecWindowPosition.y + 79 * c_menu::get().dpi_scale), ImVec2(vecWindowPosition.x + (693 + 150) * c_menu::get().dpi_scale, vecWindowPosition.y + 425 * c_menu::get().dpi_scale), ImColor(0, 0, 0, 100));
				ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(vecWindowPosition.x + (690 + 150) * c_menu::get().dpi_scale, vecWindowPosition.y + 80 * c_menu::get().dpi_scale), ImVec2(vecWindowPosition.x + (692 + 150) * c_menu::get().dpi_scale, vecWindowPosition.y + 425 * c_menu::get().dpi_scale), ImColor(aHealthBar.r(), aHealthBar.g(), aHealthBar.b(), aHealthBar.a()));
			}
			Color aAmmoBar = Color(g_cfg.player.type[0].ammobar_color);
			if (g_cfg.player.type[0].ammo)
			{
				ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(vecWindowPosition.x + (695 + 150) * c_menu::get().dpi_scale, vecWindowPosition.y + 427 * c_menu::get().dpi_scale), ImVec2(vecWindowPosition.x + (906 + 150) * c_menu::get().dpi_scale, vecWindowPosition.y + 431 * c_menu::get().dpi_scale), ImColor(0, 0, 0, 100));
				ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(vecWindowPosition.x + (695 + 150) * c_menu::get().dpi_scale, vecWindowPosition.y + 428 * c_menu::get().dpi_scale), ImVec2(vecWindowPosition.x + (905 + 150) * c_menu::get().dpi_scale, vecWindowPosition.y + 430 * c_menu::get().dpi_scale), ImColor(aAmmoBar.r(), aAmmoBar.g(), aAmmoBar.b(), aAmmoBar.a()));
			}
		...
		if (active_tab == 5)
		{
			...
			ImGui::SetCursorPos(ImVec2(186 * dpi_scale, 0)); ImGui::BeginGroup();
			{
				...
				if (ImGui::SubTab("General", subtab == 9, ImVec2(94 * dpi_scale, 53* dpi_scale)))
					subtab = 9;
			}
			ImGui::EndGroup();
			if (subtab == 9)
			{
				ImGui::PushStyleVar(ImGuiStyleVar_Alpha, (1.f - preview_alpha)* public_alpha* switch_alpha);
				 ...
				 ImGui::SetCursorPos(ImVec2(202* dpi_scale, 71* dpi_scale));
				ImGui::BeginChild("##SKINCHANGER1_FIRST", ImVec2(679* dpi_scale, 534* dpi_scale), false);
				{
					ImGui::SetCursorPosY(5);
		...
		if (active_tab == 6)
		{
			static int subtab = 10;
			ImGui::SetCursorPos(ImVec2(186 * dpi_scale, 0)); ImGui::BeginGroup();
			{
				if (ImGui::SubTab(" Main ", subtab == 10, ImVec2(94 * dpi_scale, 53* dpi_scale)))
					subtab = 10;
				ImGui::SameLine(100 * dpi_scale, 0);
				if (ImGui::SubTab("Additives", subtab == 11, ImVec2(94 * dpi_scale, 53* dpi_scale)))
					subtab = 11;
			}
			ImGui::EndGroup();
			if (subtab == 10)
			{
				 ImGui::SetCursorPos(ImVec2(202* dpi_scale, 71* dpi_scale));
				ImGui::BeginChild("##MISC1_FIRST", ImVec2(332 * dpi_scale, 534 * dpi_scale), false);
				{
					ImGui::SetCursorPosY(5);

					//ImGui::Checkbox(crypt_str("Unlock Hiden Commands"), &g_cfg.misc.hidencvars);
					...
					{
						ImGui::SliderInt(crypt_str("Ping"), &g_cfg.misc.extended_backtracking_value, 0, 200);
					}*/
					...
					if (g_cfg.misc.aspect_ratio)
					{
						...
						ImGui::SliderFloat(crypt_str("Amount"), &g_cfg.misc.aspect_ratio_amount, 0.1f, 3.0f);
					}
				...
				ImGui::EndChild();
				 ...
				 ImGui::SetCursorPos(ImVec2(550* dpi_scale, 71* dpi_scale));
				ImGui::BeginChild("##MISC2_FIRST", ImVec2(332 * dpi_scale, 534 * dpi_scale), false);
				{
					ImGui::SetCursorPosY(5);

					ImGui::Checkbox(crypt_str("Watermark"), &g_cfg.menu.watermark);

					ImGui::Checkbox(crypt_str("Keybinds"), &g_cfg.menu.bin);
					...
					ImGui::Checkbox(crypt_str("Blur"), &g_cfg.menu.blur);
			...
			{
				 ImGui::SetCursorPos(ImVec2(202* dpi_scale, 71* dpi_scale));
				ImGui::BeginChild("##MISC1_SECOND", ImVec2(332 * dpi_scale, 534 * dpi_scale), false);
				{
					ImGui::SetCursorPosY(5);
					...
					ImGui::Checkbox(crypt_str("Local trails (maybe fps drop)"), &g_cfg.esp.trails);
					ImGui::SameLine();
				...
				ImGui::EndChild();
				 ...
				 ImGui::SetCursorPos(ImVec2(550* dpi_scale, 71* dpi_scale));
				ImGui::BeginChild("##MISC2_SECOND", ImVec2(332 * dpi_scale, 534 * dpi_scale), false);
				{
					ImGui::SetCursorPosY(5);
					...
					ImGui::Checkbox(crypt_str("Trash Talk"), &g_cfg.misc.trashtalk);
		...
		if (active_tab == 7)
		{
			static int subtab = 12;
			ImGui::SetCursorPos(ImVec2(186 * dpi_scale, 0)); ImGui::BeginGroup();
			{
				if (ImGui::SubTab("Configs", subtab == 12, ImVec2(94 * dpi_scale, 53* dpi_scale)))
					subtab = 12;
				ImGui::SameLine(100 * dpi_scale, 0);
				if (ImGui::SubTab("Lua", subtab == 13, ImVec2(94 * dpi_scale, 53* dpi_scale)))
					subtab = 13;
			}
			ImGui::EndGroup();
			if (subtab == 12)
			{
				 ImGui::SetCursorPos(ImVec2(202* dpi_scale, 71* dpi_scale));
				ImGui::BeginChild("##CFG1_FIRST", ImVec2(679* dpi_scale, 534* dpi_scale), false);
				{
					ImGui::SetCursorPosY(5);
					...
					if (ImGui::CustomButton(crypt_str("Open configs folder"), crypt_str("##CONFIG__FOLDER"), ImVec2(457 * dpi_scale, 26 * dpi_scale)))
					{
						std::string folder;
					...
					if (ImGui::CustomButton(crypt_str("Refresh configs"), crypt_str("##CONFIG__REFRESH"), ImVec2(457 * dpi_scale, 26 * dpi_scale)))
					{
						cfg_manager->config_files();
						files = cfg_manager->files;
					...
					if (ImGui::CustomButton(crypt_str("Create config"), crypt_str("##CONFIG__CREATE"), ImVec2(457 * dpi_scale, 26 * dpi_scale)))
					{
					...
					if (!next_save)
					{
						...
						if (ImGui::CustomButton(crypt_str("Save config"), crypt_str("##CONFIG__SAVE"), ImVec2(457 * dpi_scale, 26 * dpi_scale)))
						{
					...
					else
					{
						...
						if (ImGui::CustomButton(crypt_str("Are you sure?"), crypt_str("##AREYOUSURE__SAVE"), ImVec2(457 * dpi_scale, 26 * dpi_scale)))
						{
							save_config();
							prenext_save = true;
							clicked_sure = true;
					...
					if (ImGui::CustomButton(crypt_str("Load config"), crypt_str("##CONFIG__LOAD"), ImVec2(457 * dpi_scale, 26 * dpi_scale)))
						load_config();
					...
					if (!next_delete)
					{
						...
						if (ImGui::CustomButton(crypt_str("Remove config"), crypt_str("##CONFIG__delete"), ImVec2(457 * dpi_scale, 26 * dpi_scale)))
						{
					...
					else
					{
						...
						if (ImGui::CustomButton(crypt_str("Are you sure?"), crypt_str("##AREYOUSURE__delete"), ImVec2(457 * dpi_scale, 26 * dpi_scale)))
						{
							remove_config();
							prenext_delete = true;
			...
			{
				 ImGui::SetCursorPos(ImVec2(202* dpi_scale, 71* dpi_scale));
				ImGui::BeginChild("##LUA1_FIRST", ImVec2(332 * dpi_scale, 534 * dpi_scale), false);
				{
					ImGui::SetCursorPosY(5);
					...
					if (ImGui::CustomButton(crypt_str("Open scripts folder"), crypt_str("##LUAS__FOLDER"), ImVec2(291 * dpi_scale, 26 * dpi_scale)))
					{
						...
						auto get_dir = [&folder]() -> void
						{
							...
							if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
								folder = std::string(path) + crypt_str("\\artic\\lua\\");
				...
				ImGui::EndChild();
				 ...
				 ImGui::SetCursorPos(ImVec2(550* dpi_scale, 71* dpi_scale));
				ImGui::BeginChild("##LUA2_FIRST", ImVec2(332 * dpi_scale, 534 * dpi_scale), false);
				{
					ImGui::SetCursorPosY(5);
...
IMaterial* fCreateMaterial(bool lit, const std::string& material_data)
{
	...
	auto matname = crypt_str("backmove_") + std::to_string(created);
	++created;
