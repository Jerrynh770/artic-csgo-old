				...
				if (!current_shot->hurt_player)
				{
					...
					if (current_shot->impact_hit_player)
					{
						...
						if (g_cfg.misc.events_to_log[EVENTLOG_HIT])
							eventlogs::get().add(crypt_str("Missed shot due to resolver"), true, 2);
#endif
					}
					else if (g_cfg.misc.events_to_log[EVENTLOG_HIT])
					{
						...
						if (current_shot->occlusion)
							eventlogs::get().add(crypt_str("Missed shot due to desync"), true, 2);
						else if (current_shot->shot_info.hitchance == 100)
							eventlogs::get().add(crypt_str("Missed shot due to prediction error"), true, 2);
						else if (current_shot->shot_info.hitchance == 101)
							eventlogs::get().add(crypt_str("Missed shot due to cfg issue"), true, 2);
						else if (current_shot->shot_info.backtrack_ticks > 14)
							eventlogs::get().add(crypt_str("Missed shot due to server correction"), true, 2);
						else
							eventlogs::get().add(crypt_str("Missed shot due to spread"), true, 2);
					}
				}
			}
