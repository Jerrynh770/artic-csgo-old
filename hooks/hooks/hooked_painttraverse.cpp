...
void __fastcall hooks::hooked_painttraverse(void* ecx, void* edx, vgui::VPANEL panel, bool force_repaint, bool allow_force)
{
	...
	if (set_console)
	{
		...
		m_cvar()->ConsoleColorPrintf(Color(121, 204, 0), "[+]Artic.su - Free Software \n");
		m_cvar()->ConsoleColorPrintf(Color(121, 204, 0), "[+]Made with love <3 \n");
		m_cvar()->ConsoleColorPrintf(Color(121, 204, 0), "[+]t.me/arsnwise");
	}
		...
		if (g_ctx.local()->is_alive())
		{
			...
			if (hWearables)
			{
				...
				if (pMask)
				{
					...
					pMask = make_glov1e(entry, serial);
	...
	if (reload_fonts())
	{
		...
		fonts[KEYBIND] = create_font(crypt_str("MuseoSansCyrl-300"), 16, FW_EXTRALIGHT, FONTFLAG_NONE);
		
		fonts[KEYBIND2] = create_font(crypt_str("MuseoSansCyrl-500"), 18, FW_LIGHT, FONTFLAG_NONE);

		fonts[KEYBIND1] = create_font(crypt_str("untitled-font-1"), 18, FW_LIGHT, FONTFLAG_NONE);
	...
	if (panel_id == panel)
	{
		...
		if (g_ctx.available())
		{
			...
			if (g_cfg.player.enable && g_cfg.esp.removals[REMOVALS_SCOPE] && is_scoped && !g_cfg.esp.custom_scopes_line)
			{
				static int w, h;
