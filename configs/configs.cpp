...
void C_ConfigManager::setup()
{
	...
	setup_item(&g_cfg.ragebot.autowall_type, false, crypt_str("Ragebot.lagexploit"));
	setup_item(&g_cfg.ragebot.double_tap, false, crypt_str("Ragebot.double_tap"));

	setup_item(&g_cfg.ragebot.peej, true, crypt_str("Ragebot.peej"));
	setup_item(&g_cfg.ragebot.slow_teleport, false, crypt_str("Ragebot.slow_teleport"));

	setup_item(&g_cfg.ragebot.speedik, 0.0f, crypt_str("Ragebot.body_scale"));
	...
	setup_item(&g_cfg.antiaim.pitch_exploit, 0, crypt_str("antiaim.pitch_exploit"));
	setup_item(&g_cfg.antiaim.pitch_exploit_key, key_bind(HOLD), crypt_str("antiaim.pitch_exploit_key"));
	...
	setup_item(&g_cfg.player.maskchanger, 0, crypt_str("player.maskchanger"));
	...
	setup_item(&g_cfg.antiaim.flicktick, 5, crypt_str("Player.flick"));
	setup_item(&g_cfg.antiaim.flick, false, crypt_str("Player.flickenable"));
	...
	setup_item(&g_cfg.warning.main.d_warn_type, 0, crypt_str("warning.iuoy"));
	setup_item(&g_cfg.warning.main.visible_only, false, crypt_str("warning.hhjgfh"));

	setup_item(&g_cfg.warning.main.show_bg, false, crypt_str("warning.213"));
	setup_item(&g_cfg.warning.main.show_damage_dist, false, crypt_str("warning.d f"));
	setup_item(&g_cfg.warning.main.show_icon, 0, crypt_str("warning.z"));
	setup_item(&g_cfg.warning.main.show_timer, false, crypt_str("warning.asd"));
	setup_item(&g_cfg.warning.main.color_by_time, false, crypt_str("warning.warn123_col"));
	...
	setup_item(&g_cfg.misc.trashtalk, false, crypt_str("Misc.trashtalk"));
	setup_item(&g_cfg.scripts.developer_mode, false, crypt_str("scripts.developer_mode"));
	setup_item(&g_cfg.scripts.allow_http, false, crypt_str("scripts.allow_http"));
	setup_item(&g_cfg.scripts.allow_file, false, crypt_str("scripts.allow_file"));
	setup_item(&g_cfg.misc.extended_backtracking, false, crypt_str("misc.extended_bactracking"));
	setup_item(&g_cfg.misc.extended_backtracking_value, false, crypt_str("misc.extended_bactracking_value"));
	setup_item(&g_cfg.misc.hidencvars, false, crypt_str("misc.hiddencvars"));
	...
	setup_item(&g_cfg.menu.bin, false, crypt_str("Menu.keybinds"));
	setup_item(&g_cfg.menu.blur, false, crypt_str("Menu.blur"));
...
void C_ConfigManager::save(std::string config)
{
	...
	auto get_dir = [&folder, &file, &config]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
		{
			...
			file = std::string(path) + crypt_str("\\artic\\Configs\\") + config;
		}
