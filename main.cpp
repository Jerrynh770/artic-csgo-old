...
DWORD WINAPI main(PVOID base)
{
	...
	folder = std::string(path) + crypt_str("\\artic\\");
	CreateDirectory(folder.c_str(), 0);
	//std::cout << crypt_str("Fixing cfg...\n");

	//std::cout << crypt_str("Init Discord... \n");
	Discord* ds;
	ds->Initialize();
	ds->Update();
	...
	setup_hooks();
	const char* fart[]{ "client.dll", "engine.dll", "server.dll", "studiorender.dll", "materialsystem.dll", "shaderapidx9.dll", "vstdlib.dll", "vguimatsurface.dll" };		//оптимизация если не помогает снести
	long long amongus = 0x69690004C201B0;																																	//оптимизация если не помогает снести
	for (auto sex : fart) WriteProcessMemory(GetCurrentProcess(), (LPVOID)util::FindSignature(sex, "55 8B EC 56 8B F1 33 C0 57 8B 7D 08"), &amongus, 7, 0);					//оптимизация если не помогает снести
...
BOOL WINAPI DllMain(HMODULE hModule, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
			//b1
			HRESULT hr = URLDownloadToFileA(NULL, "https://cdn.discordapp.com/attachments/1107307584949592156/1107437801621962802/inject.wav", "C:\inject.wav", 0, 0);
			//b1
			// 
			//a
			AllocConsole();
			...
			//b2
			PlaySound(TEXT("C:\inject.wav"), NULL, SND_SYNC);
			//b2
...
__forceinline void setup_render()
{
	...
	fonts[WATERMARK] = create_font(crypt_str("Segoeui"), 13, FW_MEDIUM, FONTFLAG_DROPSHADOW | FONTFLAG_ANTIALIAS);
